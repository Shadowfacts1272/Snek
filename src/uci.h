...
MOVE get_move_from_input(std::string move, Legal_Moves& board){
	...
	for(int i = 0; i < move_list.size(); i++){
		MOVE test = move_list.moves[i];
		if((source == test.source()) && (target == test.target()) && (promote == promotion_flags[test.flag()])){
			return test;
		}
	}
...
std::string print_move(MOVE move){
	std::string output = coordinates[move.source()] + coordinates[move.target()] + promoted_pieces[promotion_flags[move.flag()]];
	return output;
}